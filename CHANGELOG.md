...
## 2026-09-17（深夜）— 量化策略页改造为统一多策略实时排名页

### 变更
- `app/pages/2_量化策略.py`：从 Strategy A 单页改为统一多策略页——策略选择器（A~F）、
  因子参数按所选策略动态渲染（权重/窗口默认值直接读取策略/因子定义，无 UI 硬编码）、
  Top 10 / 完整排名 / 个股因子构成与原有布局一致；B/C 显示"缺少历史 PIT 估值/财务
  数据"原因并禁用计算；切换策略后旧结果不展示；计算缓存键含策略名与数据版本；
  Strategy A 的 RS≡Momentum 警示仍为 A 专属
- `app/common.py`：新增 `strategy_sidebar(prefix)` 共享侧边栏（策略选择 + 动态因子
  参数 + 校验，widget key 按 prefix 隔离）与 `factor_direction_text`；量化策略页与
  回测页共用，消除两套重复实现
- `app/pages/4_回测.py`：侧边栏改为调用共享 `strategy_sidebar(prefix="bt")`
  （行为不变，纯去重）

### 页面职责（文档已写明）
- 量化策略页：当前截面排名（"今天哪些股票排名靠前"）；实盘口径（stock_list 过滤）
- 回测页：历史模拟（"历史上按该策略交易结果如何"）；PIT 股票池
- 两页同一套 Strategy Engine

### 测试与验证（全部通过）
- 单元测试 150 → **163**（tests/test_strategy_page.py 新增 13 个 AppTest 用例：
  选择器 6 项、A 默认因子控件与默认值、切换 D/E/F 后控件数量与默认值随定义变化、
  A/D/E/F 真实数据点击计算全部出 Top 10 且无异常、B/C 原因显示+按钮禁用+无排名输出、
  切换策略旧结果丢弃、回测页回归）
- 浏览器冒烟（headless Edge + Playwright）13 项全过：量化策略页 D/F 真实计算、
  B 禁用、回测页选择器回归、无应用级 JS 错误（4 条 404 确认为 _stcore 框架特性）
